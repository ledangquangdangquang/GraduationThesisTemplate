...
% show line margin (Comment dòng trên lại để không bị trùng)
...
% XỬ LÝ ẢNH VECTOR
\usepackage{tikz} % add ảnh .svg thôi, tạo khung viền 
\usepackage{circuitikz} % vẽ mạch điện 
\usepackage{siunitx} % Để viết giá trị
\usetikzlibrary{calc}
...
\begin{document}
    % INPUT NỘI DUNG 
    \input{NoiDung/Bia} 
    \input{NoiDung/NhiemVuDoAnTotNghiep} 
    \input{NoiDung/CamOnvaTomTat} 
    \input{NoiDung/MucLuc} 
    \input{NoiDung/NoiDungCacChuong} 
    \input{NoiDung/KetLuan} 
    \input{NoiDung/TaiLieuThamKhao} 
    \input{NoiDung/PhuLuc} 
\end{document}